...
#+end_src
** DONE Remove ~custom.el~
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
(setq custom-file null-device);
#+end_src
** DONE Straight (use-package syntax)
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes

(straight-use-package 'use-package)

;; For running  M-x use-package-report
...
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
  ...
  #+end_src
** DONE Speed up with =compile-angel=
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
    ;; Speed up Emacs
...
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
  ...
  (use-package exec-path-from-shell
    ...
    :config
    (setq exec-path-from-shell-arguments nil)  ;; skip interactive shell
    (exec-path-from-shell-initialize))
#+end_src
** Emacs
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
...
#+end_src
** DONE Auto-revert, recentf, saveihst, save-place
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
...
(use-package embark
  ...
  :bind
  (("C-x ." . embark-act)         ;; pick some comfortable binding
   ("C-x ;" . embark-dwim)        ;; good alternative: M-.
...
(use-package org
  ...
  (setq org-startup-indented t ;; auto aligns text with header
        ...
        org-directory (my-first-existing-directory '("/home/shumma1/notes/2026/"
                                                     "/Users/SHUMMA1/notes/2026/"))
...
(straight-use-package
 '(prot-modeline :type git
  		       ...
  		       "emacs/.emacs.d/prot-lisp/prot-icons.el"
...
(use-package prot-modeline
  :ensure nil
  :custom
  (prot-modeline-string-truncate-length 15) ; default is 9
  :config
  (setq mode-line-compact nil) ; Emacs 28
  ;;(setq mode-line-right-align-edge 'right-margin) ;; Emacs 30, ignore for now because I want far right edge, not margin
...
#+end_src
** DONE Keycast
*** Config is used with prot-modeline
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes

;; This config is used with prot-modeline
(use-package keycast
  :defer t
  :custom
...
(add-hook 'prog-mode-hook #'rainbow-delimiters-mode)
 ...
 (menu-bar-mode -1)
 (tool-bar-mode -1)
 ;;(scroll-bar-mode -1) ;; not available in linux serverq
 ;;(setq-default line-spacing 0.4) ;; only works in gui
...
(use-package gptel
  ...
  (setq gptel-backend (gptel-make-gh-copilot "Github Copilot Enterprise"))
    
  ;; Enable thinking mode
  (setq gptel-include-reasoning "ignore")
  
  ;; Customize heading level
  (setq gptel-prompt-prefix-alist
        '((markdown-mode . "# ")))
  
  ;; Or set response prefix separately
...
(use-package mcp
  ...
  :custom (mcp-hub-servers
           `(("askit-tools" . (:url "http://nrusca-ldl30024.nibr.novartis.net:8000/mcp"))
             ("literature" . (:url "http://nrusca-ldl30024.nibr.novartis.net:8001/mcp"))
             ("structural-bioinformatics" . (:url "http://nrusca-ldl30024.nibr.novartis.net:8002/mcp"))
             ("confluence" . (:url "http://nrusca-ldl30024.nibr.novartis.net:8004/mcp"))
             ("opentargets" . (:url "http://nrusca-ldl30024.nibr.novartis.net:8006/mcp"))
             ;;("visualizer" . (:url "https://nrusca-ldl30024.nibr.novartis.net:8003/mcp"))
             ("hithub-mcp" . (:url "https://hithub.prd.nibr.novartis.net/mcp/"
                              :env (:USER "shumma1")))
             ("cdf-mcp" . (
                           :command "uv"
                           :args ("run" "--directory" ,(expand-file-name "~/work/sandbox/new_cdf_sandbox") "-m" "cdf_api_mcp")
                           :env (
                                 :CDF_URL "https://cdf.prd.nibr.novartis.net"
                                 :HTTP_PROXY ""
                                 :HTTPS_PROXY ""
                                 :CDF_CA_BUNDLE ,(expand-file-name "~/.config/requests/cert.pem")
                                 :NO_PROXY ".novartis.net,.nibr.novartis.net,localhost,127.0.0.1"
                                 )
                           )
             )))
  :config
...
(use-package flymake
  :straight (:type built-in)
  :custom
  (flymake-indicator-type 'fringes)  ; Use fringes instead of margin
  :hook
...
#+end_src
** Tree-Sitter
- Tree-sitter is built-in for Emacs 30 (using "treesit")
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
  ...
  (add-to-list 'treesit-extra-load-path "/home/shumma1/.emacs.d/var/tree-sitter/")
...
#+begin_src emacs-lisp :tangle "post-init.el" :mkdirp yes
  (use-package olivetti
    :straight t
    ;;:hook (org-mode . olivetti-mode) ;; turn off
    :custom
...
(set-face-attribute 'default nil
                    :height 170 :weight 'normal :family "Iosevka Term")
...
(use-package kitty-graphics
  ...
  :straight (:host github :repo "cashmeredev/kitty-graphics.el" :files ("*.el"))
  :config
  (setq kitty-gfx-enable-video nil)   ; optional: inline mpv playback
